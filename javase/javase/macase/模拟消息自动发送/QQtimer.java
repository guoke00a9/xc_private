...
public class QQtimer {
	public static void main(String[] args) {
		...
		timer.schedule(new TimerTask() {
			public void run() {
				String sentence = "everybody早上好,我是你们帅气又迷人的罡罡哥哥,今夜阳光明媚,今天多云转晴,早安、早安";// 定义要发送的话
				Robot robot = null;
				try {
					robot = new Robot();
				...
				// for (int j = 1; j <= 1; j++) {undefined
				// 以下两行按下了ctrl+v，完成粘贴功能
						robot.keyPress(KeyEvent.VK_CONTROL);
						robot.keyPress(KeyEvent.VK_V);
						robot.keyRelease(KeyEvent.VK_CONTROL);
						/**
						 * 释放ctrl按键，像ctrl，退格键，删除键这样的功能性按键，
						 * 在按下后一定要释放，不然会出问题。crtl如果按住没有释放，
						 * 在按其他字母按键是，敲出来的回事ctrl的快捷键。
						 */
						robot.delay(1000);
						/** 延迟一秒再发送，不然会一次性全发布出去，因为电脑的处理速度很快，每次粘贴发送的速度几乎是一瞬间，所以
						 * 给人的感觉就是一次性发送了全部。这个时间可以自己改，想几秒发送一条都可以
						 */
